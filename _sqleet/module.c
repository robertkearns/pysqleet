...
PyObject* pysqlite_Error, *pysqlite_Warning, *pysqlite_InterfaceError, *pysqlite_DatabaseError,
    ...
    *pysqlite_IntegrityError, *pysqlite_DataError, *pysqlite_NotSupportedError, *pysqleet_AuthenticationError;
        ...
        kwargs)
{
    ...
    static char *kwlist[] = {
        ...
        "check_same_thread", "factory", "cached_statements", "uri", "key",
        NULL
    };
    char* database;
    int detect_types = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s|diOiOipz#", kwlist,
                                     ...
                                     &factory, &cached_statements, &uri, &key, &keyLen))
    {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit__sqleet(void)
{
    ...
    if (!(pysqleet_AuthenticationError = PyErr_NewException(MODULE_NAME ".AuthenticationError", pysqlite_DatabaseError, NULL))) {
        goto error;
    }
    PyDict_SetItemString(dict, "AuthenticationError", pysqleet_AuthenticationError);
