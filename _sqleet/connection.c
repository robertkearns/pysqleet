 ...
 * Copyright (C) 2021 Robert Kearns <robert@celrum.com>
 *
 * This file is a modified version of pysqlite for integration with sqleet
 *
...
int pysqlite_connection_init(pysqlite_Connection* self, PyObject* args, PyObject* kwargs)
{
    ...
    if (key && pysqleet_set_key(self->db, key, keyLen) < 0) {
        return -1;
    }
    ...
    self->AuthenticationError   = pysqleet_AuthenticationError;
    return 0;
}
...
/*
 * Called to set the key on either a new fresh database, or to pass the key on an encrypted database which was
 * just opened. Returns 1 on success. On failure sets the error
 */
static int
pysqleet_set_key(sqlite3* db, const char *key, Py_ssize_t keyLen)
{
    ...
    if (rc == SQLITE_AUTH) {
        PyErr_SetString(pysqleet_AuthenticationError, "Passed key is incorrect");
        return -1;
    ...
    // Run a basic query to check if key is correct. If the key is incorrect, we will get "not a database error".
    // This could also happen of course if the file is actually not a database.
...
/*
 * Called to change the key on an existing database.
 */
static PyObject *
pysqleet_change_key(pysqlite_Connection* self, PyObject* args)
{
    const char *key;
    Py_ssize_t keyLen;

    if (!PyArg_ParseTuple(args, "z#", &key, &keyLen)) {
        return NULL;
    }
...
static PyMethodDef connection_methods[] = {
    ...
    {"change_key", (PyCFunction)pysqleet_change_key, METH_VARARGS,
        PyDoc_STR("Rekeys the entire database with the new key.")},
